...
exports.onUserDeleted = functions.auth.user().onDelete(async (user) => {
  ...
  await firestore
    ...
    .then(async (querySnapshot) => {
      for (var doc of querySnapshot.docs) {
        console.log(`Deleting document ${doc.id} from collection activityList`);
        await doc.ref.delete();
      }
    });
  ...
  await firestore
    ...
    .then(async (querySnapshot) => {
      for (var doc of querySnapshot.docs) {
        console.log(`Deleting document ${doc.id} from collection contactList`);
        await doc.ref.delete();
      }
    });
  await firestore
    ...
    .then(async (querySnapshot) => {
      for (var doc of querySnapshot.docs) {
        console.log(`Deleting document ${doc.id} from collection routeList`);
        await doc.ref.delete();
      }
    });
  await firestore
    .collection("notifications")
    .where("contactList", "array-contains", user.uid)
    .get()
    .then(async (querySnapshot) => {
      for (var doc of querySnapshot.docs) {
        console.log(
